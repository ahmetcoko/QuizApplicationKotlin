...
import com.example.quizapplication.Adapter.LeaderAdapter
import com.example.quizapplication.Domain.UserModel
import com.example.quizapplication.R
import com.example.quizapplication.databinding.ActivityLeaderBinding
...
class LeaderActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.apply {
            ...
            val drawableResourceId1 :Int = binding.root.resources.getIdentifier(loadData().get(0).pic,"drawable",binding.root.context.packageName)

            val drawableResourceId2 :Int = binding.root.resources.getIdentifier(loadData().get(1).pic,"drawable",binding.root.context.packageName)


            val drawableResourceId3 :Int = binding.root.resources.getIdentifier(loadData().get(2).pic,"drawable",binding.root.context.packageName)


            Glide.with(root.context).load(drawableResourceId1).into(shapeableImageView1)
            Glide.with(root.context).load(drawableResourceId2).into(shapeableImageView2)
            Glide.with(root.context).load(drawableResourceId3).into(shapeableImageView3)
    ...
    private fun loadData():MutableList<UserModel>{
        ...
        users.add(UserModel(7,"Sarah Wilson" , "person9" , 2870))
